        ...
        <div className="flex-1 overflow-hidden rounded-2xl border border-white/10 bg-slate-800/50 backdrop-blur-sm">
                    ...
                    <tbody className="divide-y divide-white/5">
                        {data.map((ins) => (
                            ...
                            >
                                ...
                                <td className="whitespace-nowrap px-5 py-4">
                                    <span
                                        className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold ring-1 ${ins.status === InsuranceStatus.ACTIVE
                                                ? 'bg-emerald-500/10 text-emerald-400 ring-emerald-500/20'
                                                : 'bg-rose-500/10 text-rose-400 ring-rose-500/20'
                                            }`}
                                    >
                                        <span
                                            className={`h-1.5 w-1.5 rounded-full ${ins.status === InsuranceStatus.ACTIVE
                                                    ? 'bg-emerald-400'
                                                    : 'bg-rose-400'
                                                }`}
                                        />
            ...
            <div className="flex items-center justify-between border-t border-white/10 px-5 py-3">
                <p className="text-xs text-slate-400">
                    <span className="font-semibold text-white">{totalItems}</span> convênios encontrados
                </p>
